 ...
 if(isset($_GET['postUserLogin']))
{
	if($_POST['phone']!=="" and $_POST['password']!=="")
	{
		...
		{ 
			...
			$set['JSON_DATA'][]	= array(  
						  'msg'			=>	'Welcome back to the PrizeX Club',
						  'success'=>'1',
						  'id' 	=>	$row['id'],
 						  'name'	=>	$row['name'],
 						  'email'	=>	$row['email'],
		...
		{
			$set['JSON_DATA'][]=array('msg' =>'Invalid Mobile Number or Password','success'=>'0');
...
//INSERT INTO `tbl_offers`(`o_id`, `o_name`, `o_image`, `o_desc`, `o_time`, `o_status`) 
		while($data = mysqli_fetch_assoc($sql))
		{
			...
			$row['o_oimage'] =  $file_path.'images/'.$data['o_oimage'];
			$row['o_currency'] = $data['o_currency'];
			$row['o_stars'] = $data['o_stars'];
			$row['o_membership'] = $data['o_membership'];
			$row['o_qty'] = $data['o_qty'];
			$row['o_cat'] = $data['o_cat'];
			$row['o_umax'] = $data['o_umax'];
			$row['o_ulimit'] = $data['o_ulimit'];
...
//get items
else if(isset($_GET['get_items']))
  	{
		 ...
		 $query13= "SELECT * FROM tbl_offers left join tbl_cat on tbl_cat.c_id= tbl_offers.c_id WHERE ( o_date <= '".$date1."' and o_edate >= '".$date1."' ) and o_status = 1 and c_status='1' ORDER BY `tbl_offers`.`o_id` ASC";
		$sql = mysqli_query($mysqli,$query13)or die(mysqli_error());
...
//get views
else if(isset($_GET['get_views']))
 	{
		...
		$query="SELECT * FROM tbl_views 
		left join tbl_users on tbl_users.id= tbl_views.u_id
		WHERE  (_date <= '".$date1."')  AND u_id='".$_POST['u_id']."' ";
		$sql = mysqli_query($mysqli,$query)or die(mysqli_error());
...
//get all offers live
else if(isset($_GET['get_offers_live']))
 	{
		 ...
		 $query="SELECT * FROM tbl_offers WHERE ( o_date <= '".$date1."' and o_edate >= '".$date1."' ) and o_status = 1 ORDER BY `tbl_offers`.`o_stime` ASC";
		$sql = mysqli_query($mysqli,$query)or die(mysqli_error());
...
//get all offers upcomming
else if(isset($_GET['get_offers_upcomming']))
 	{
        ...
        $datetime = date('Y-m-d H:i:s');
		 ...
		 $query="SELECT * FROM tbl_offers WHERE  o_date >= '".$date1."' and o_status = 1 ORDER BY `tbl_offers`.`o_stime` ASC";
		$sql = mysqli_query($mysqli,$query)or die(mysqli_error());
 ...
 where tbl_bid.o_id='".$data['o_id']."' ";
  ...
  //get all offers winner
	...
	date_default_timezone_set("Asia/Calcutta");   //India time (GMT+5:30)
	 ...
	 $query="SELECT * FROM tbl_offers WHERE o_edate <= '".$date1."' and o_type = 1 OR o_type = 2 OR o_type = 3 OR o_type = 4  and o_status = 1 ORDER BY `tbl_offers`.`o_edate` DESC";
...
//get all offers
else if(isset($_GET['get_order_user']))
 	{
		...
		WHERE u_id='".$_POST['u_id']."' ORDER BY `tbl_order`.`o_id` DESC";
		$sql = mysqli_query($mysqli,$query)or die(mysqli_error());
             ...
             $qry1 = "SELECT * FROM tbl_users WHERE phone = '".$phone."' and status='1'"; 	 
		        ...
		        $password = $row1['password'];
        	        ...
        	        if ($num_rows > 0 )
        			{
                        ...
                        $YourAPIKey='e0a89fa3-1835-11eb-b380-0200cd936042';
                        $agent= 'Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1; SV1; .NET CLR 1.0.3705; .NET CLR 1.1.4322)';
                        
                        $url = "https://2factor.in/API/R1/?module=TRANS_SMS&apikey=$YourAPIKey&to=$phone&from=PrizeX&templatename=registerotp&var1=$text";
        			   ...
        			   if($result['Status'] == "Success"){
        				   $set['JSON_DATA'][]=array('msg' => "Please accept the Privacy Policy to Continue",'success'=>'1');
        			   }
        			   else{
...
else if(isset($_GET['get_winner']))
{
	  $jsonObj4= array();	

   $query="SELECT * FROM tbl_raffle WHERE w_status='1' ORDER BY w_id DESC ";
   $sql = mysqli_query($mysqli,$query)or die(mysqli_error());
