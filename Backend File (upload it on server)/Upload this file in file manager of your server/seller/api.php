 ...
 if(isset($_GET['postUserLogin']))
{
	if($_POST['phone']!=="" and $_POST['password']!=="")
	{
		...
		{ 
			...
			$set['JSON_DATA'][]	= array(  
						  'msg'			=>	'Welcome back to the Auction App',
						  'success'=>'1',
						  'id' 	=>	$row['id'],
 						  'name'	=>	$row['name'],
 						  'email'	=>	$row['email'],
...
//get all offers
else if(isset($_GET['get_bid_user']))
 	{
		...
		WHERE u_id='".$_POST['u_id']."' ORDER BY `tbl_bid`.`bd_id` DESC";
		$sql = mysqli_query($mysqli,$query)or die(mysqli_error());
...
//get all transaction
 else if(isset($_GET['get_transaction']))
 	{
		...
		while($data = mysqli_fetch_assoc($sql))
		{
 			    ...
 			    if($data['type'] == 3)
 			    {
 			        $qry3 = "SELECT * FROM tbl_users WHERE id = '".$data['type_no']."'"; 
            		$result3 = mysqli_query($mysqli,$qry3);
            		$row3 = mysqli_fetch_assoc($result3);
            		
            		$row['type_name'] = "Won" ;
            		$row['type_details'] = $row3['name'] ;
            	    	$row['type_image'] = $row1['image'] ;
 			    }
 			     if($data['type'] == 4)
 			    {
 			        $qry4 = "SELECT * FROM tbl_users WHERE id = '".$data['type_no']."'"; 
            		$result4 = mysqli_query($mysqli,$qry4);
            		$row4 = mysqli_fetch_assoc($result4);
            		
            		$row['type_name'] = "Lost" ;
            		$row['type_details'] = $row4['name'] ;
            	    	$row['type_image'] = $row1['image'] ;
 			    }
